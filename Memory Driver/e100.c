...
void pci_epc_unmapaddr(struct pci_epc *epc, u8 func_no,
			phys_addr_t phys_addr)
{
	...
	mutex_lock((unsigned long) &epc->lock);
	epc->ops->unmap_addr(epc, phys_addr);
	mutex_unlock((unsigned long) &epc->lock);
}

int pci_epc_mapaddr(struct pci_epc *epc, u8 func_no,
...
static int pci_epf_test_datatransfer(struct pci_epf_test *epf_test,
...
static int pci_epftestread(struct pci_epf_test *epf_test)
{
	int ret;
	...
	ret = pci_epc_mapaddr(epc, epf->func_no, phys_addr, reg->src_addr,
			       reg->size);
	if (ret) {
	...
	if (use_dma) {
		...
		ret = pci_epf_test_datatransfer(epf_test, dst_phys_addr,
						 phys_addr, reg->size);
		if (ret)
...
err_map_addr:
	pci_epc_unmapaddr(epc, epf->func_no, phys_addr);
...
static int pci_epf_test_write(struct pci_epf_test *epf_test)
{
	...
	ret = pci_epc_mapaddr(epc, epf->func_no, phys_addr, reg->dst_addr,
			       reg->size);
	if (ret) {
		dev_err(dev, "Failed to map address\n");
		reg->status = STATUS_DST_ADDR_INVALID;
	...
	if (use_dma) {
		...
		ktime_get_ts64(&start);
		ret = pci_epf_test_datatransfer(epf_test, phys_addr,
...
static int pci_epf_test_setbar(struct pci_epf *epf)
{
	int bar, add;
	int ret;
	...
	for (bar = 0; bar < PCI_STD_NUM_BARS; bar += add) {
		epf_bar = &epf->bar[bar];
		/*
		 * pci_epc_setbar() sets PCI_BASE_ADDRESS_MEM_TYPE_64
